...
local Actor = requireFW("src.global.functions.class")("Actor")
...
function Actor.createModel(name, ...)
	if (not name) then
		...
		return requireFW(MODEL_PATH .. name):create(...)
	end
end
...
function Actor.createView(name, ...)
	if (not name) then
		...
		return requireFW(VIEW_PATH .. name):create(...)
	end
end
...
function Actor.createWithModelAndViewName(modelName, modelParam, viewName, viewParam)
	...
	local view = (Actor.createView( viewName,  viewParam)) or (nil)
