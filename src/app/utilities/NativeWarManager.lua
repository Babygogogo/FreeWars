...
local function generatePlayersData(warFieldData, playerIndex, startingEnergy, startingFund)
	local data = {[playerIndex] = generateSinglePlayerData("Player", playerIndex, 1, startingEnergy, startingFund)}
	for i = 1, warFieldData.playersCount do
		data[i] = data[i] or generateSinglePlayerData("A.I.", i, i, startingEnergy, startingFund)
	end
	return data
end
...
function NativeWarManager.createInitialWarData(warConfiguration)
	return {
		...
		players  = generatePlayersData(warConfiguration.warFieldData, warConfiguration.playerIndex, warConfiguration.startingEnergy, warConfiguration.startingFund),
		turn	 = TableFunctions.clone(DEFAULT_TURN_DATA),
		warField = {warFieldFileName = warConfiguration.warFieldFileName},
		warFieldFileName=warConfiguration.warFieldFileName,
		warFieldData=warConfiguration.warFieldData
	}
end
