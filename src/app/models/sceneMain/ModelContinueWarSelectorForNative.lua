...
local function createWarList(self, warConfigurations)
	...
	for _, warConfiguration in pairs(warConfigurations) do
		...
		warList[#warList + 1] = {
			...
			warFieldName = (warField and warField.warFieldName) or '????',
			callback	 = function()
				getActorWarFieldPreviewer(self):getModel():setWarField(warField)
					:setLeftLabelText(generateLeftLabelText(warConfiguration))
					:setEnabled(true)
				self.m_View:setButtonNextVisible(warField~=nil)
