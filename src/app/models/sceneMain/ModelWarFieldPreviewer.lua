...
local function initViewUnitMap(self, warFieldFileName)
    local layer               = requireFW("res.data.templateWarField." .. warFieldFileName).layers[3]
...
function ModelWarFieldPreviewer:setWarField(warFieldFileName)
    if (self.m_WarFieldFileName ~= warFieldFileName) then
        ...
        if (isRandomWarField(warFieldFileName)) then
            self.m_RandomPlayersCount = requireFW("res.data.templateWarField." .. warFieldFileName).playersCount
        else
        ...
        self.m_View:setAuthorName(requireFW("res.data.templateWarField." .. warFieldFileName).authorName)
    end
